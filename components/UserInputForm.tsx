...
const UserInputForm: React.FC<UserInputFormProps> = ({ userInputs, setUserInputs, onSubmit, isLoading }) => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
      ...
      <p className="text-center text-gray-500 mb-8">Crie atividades divertidas e educativas!</p>
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="space-y-6">
        <div>
          <label htmlFor="className" className="block text-sm font-medium text-gray-700 mb-1">Nome do Aluno/Turma</label>
          <input
            type="text"
            id="className"
              ...
              <button
                ...
                className={`text-center py-3 px-2 rounded-lg border-2 transition-transform duration-150 transform hover:scale-105 ${
                  userInputs.activityType === type
                    ? 'bg-teal-600 text-white border-teal-600 font-semibold shadow-md scale-105'
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-teal-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center transition-colors duration-300"
        >
