...
const getFunFact = () => {
  const facts = [
    "Gerando uma cruzadinha desafiadora...",
    "Construindo uma imagem ponto a ponto...",
    "Montando um caça-palavras estimulante...",
    "Ativando os algoritmos de criatividade...",
    "Processando a sua solicitação...",
    "Gerando conteúdo educacional personalizado...",
    "Compilando as palavras para a atividade..."
  ];
        ...
        <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
            ...
            <div className="flex flex-col justify-center">
                ...
                <ul className="flex flex-wrap justify-center gap-2">
                    {data.words.map(word => 
                        <li key={word} className="font-semibold bg-teal-100 text-teal-800 px-3 py-1 rounded-full text-base">
                            {word}
                        </li>
                    )}
                </ul>
...
const CoverPage: React.FC<{ data: CoverPageData }> = ({ data }) => {
    return (
        <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-teal-50 text-center">
            <div className="border-4 border-slate-700 p-4 w-full h-full flex flex-col items-center justify-around border-double">
                 <div>
                    <h1 className="text-5xl font-bold text-slate-800">Caderno de Atividades</h1>
...
const ActivityDisplay: React.FC<ActivityDisplayProps> = ({ pages, onReset, isLoading }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    pdf.save('caderno-de-atividades.pdf');
  ...
  if (isLoading) {
    return (
        <div className="text-center p-8">
            <SpinnerIcon className="animate-spin h-12 w-12 mx-auto text-teal-600" />
    ...
    <div className="w-full">
      <div className="flex justify-center items-center gap-4 my-8 sticky top-4 z-10">
        <button
          ...
          className="bg-green-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:bg-gray-400 flex items-center gap-2 transition-colors"
        >
