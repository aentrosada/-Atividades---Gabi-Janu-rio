  ...
  switch (activityType) {
    case ActivityType.CONNECT_THE_DOTS:
      return {
        type: Type.OBJECT,
        properties: {
          title: { type: Type.STRING, description: "Um título criativo para o desenho." },
          points: {
            type: Type.ARRAY,
...
const getPrompt = (inputs: UserInputs, instance: number): string => {
  ...
  const base = `Gere uma atividade educativa. Idioma: ${inputs.language}. Faixa etária: ${inputs.age}. Tema: "${inputs.theme}". Atividade: ${inputs.activityType}.`;
